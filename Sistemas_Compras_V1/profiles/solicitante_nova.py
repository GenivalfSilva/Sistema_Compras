...
def nova_solicitacao(data: Dict, usuario: Dict, USE_DATABASE: bool = False):
    ...
    if not catalogo:
        ...
        # Informação sobre gerenciamento de produtos (removido conforme solicitação do cliente)
        st.info("ℹ️ **Gerenciamento de Produtos**: O cadastro de produtos é realizado pelos perfis Admin e Suprimentos para manter a padronização.")
    ...
    with st.form("nova_solicitacao_final"):
        ...
        with col5:
            st.text_input("Nº da Solicitação", value="Será gerado automaticamente", disabled=True)
